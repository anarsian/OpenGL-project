...
public:
	...
	void  operator*(const Matrix4& m2); //multiply matrix with matrix
	void  operator*( Vector4& v); //multiply matrix with vector
	void  makeRotateX(double angle); //make rotation matrix about X axis with angle in degrees (note that the sin/cos functions in C++ expect radians so you need to convert to radians within the function)
	...
	void  makeRotate(double angle, const Vector4& axis); //Make a rotation matrix about an arbitrary axis
